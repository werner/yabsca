...
wc.Label.prototype.create = function (r, x, y) {
	...
	var t = this;
	if (t.font() == undefined)
		this.g = t.r().text(t.x(), t.y(), t.text()).attr({fill:"#fff", stroke:"#000", "font-family":t.fontfamily(), "font-size":t.fontsize(), 'letter-spacing':t.letterspacing(), rotation:this.rotation() + " " + this.x() + " " + this.y(), "text-anchor":this.align()});
	else 
		this.g = t.r().text(t.x(), t.y(), t.text()).attr({fill:"#fff", stroke:"#000", font: t.font(), "line-spacing":t.letterspacing(), rotation:this.rotation() + " " + this.x() + " " + this.y(), "text-anchor":this.align()});
	return this;
}
...
wc.CGauge.prototype.drawDial = function (tick, length, isLargeTick) {
	...
	for (var i = 0; i <= n; i++) {
		var ang = (minAngle + startAngle + i * tickAngle);
		r.path("M" + cx + " " + (cy + radius) + "L" + cx+ " " + (cy + radius - length)).attr({rotation: ang + " " + cx + " " + cy, "stroke-width": isLargeTick? 2 : 1, stroke: "#000"});		
		if (isLargeTick) 
		{
			...
			if (ang >= 90 && ang <= 270) {
				if (minValAlt + i * tick == 0)
					r.text(cx, cy - radius - this.labelOffset(), "0").attr({rotation:(ang-180) + " " + cx + " " + cy, "stroke-width": 1, stroke: "#000", "font-size":this.labelFontSize(), fill: "#000"});						
				else 
					r.text(cx, cy - radius - this.labelOffset(), minValAlt + i * tick).attr({rotation:(ang-180) + " " + cx + " " + cy, "stroke-width": 1, stroke: "#000", "font-size":this.labelFontSize(), fill: "#000"});						
			}
			else {
				if (minValAlt + i * tick == 0)
					r.text(cx, cy + radius + this.labelOffset(), "0").attr({rotation:ang + " " + cx + " " + cy, "stroke-width": 1, stroke: "#000", "font-size":this.labelFontSize(), fill: "#000"});
				else 
					r.text(cx, cy + radius + this.labelOffset(), minValAlt + i * tick).attr({rotation:ang + " " + cx + " " + cy, "stroke-width": 1, stroke: "#000", "font-size":this.labelFontSize(), fill: "#000"});
			}
		}
	}
}
